...
import { convertToDollarUnit, formatCurrency } from "../utils/format-currency";
...
export const updateReportSettingService = async (
  ...
  // India time zone ke saath log
  const istTime = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
  console.log(`[${istTime}] Next report date:`, nextReportDate);
...
export const generateReportService = async (
  ...
  // India time zone ke saath log
  const istTime = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
  console.log(`[${istTime}] Report generated:`, { totalIncome, totalExpenses, categories });
  ...
  // 🇮🇳 INDIA DATE FORMAT - Changed here
  const periodLabel = `${format(fromDate, "dd MMMM yyyy")} - ${format(toDate, "dd MMMM yyyy")}`;
  // Alternative: "dd/MM/yyyy" format
  // const periodLabel = `${format(fromDate, "dd/MM/yyyy")} - ${format(toDate, "dd/MM/yyyy")}`;
  ...
  const incomeInRupees = convertToDollarUnit(totalIncome);
  const expensesInRupees = convertToDollarUnit(totalExpenses);
  const balanceInRupees = convertToDollarUnit(availableBalance);

  return {
    period: periodLabel,
    summary: {
      income: incomeInRupees,
      expenses: expensesInRupees,
      ...
      incomeFormatted: formatCurrency(incomeInRupees),
      expensesFormatted: formatCurrency(expensesInRupees),
      balanceFormatted: formatCurrency(balanceInRupees),
    },
    insights,
  };
};
